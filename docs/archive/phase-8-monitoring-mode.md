...
**Variant B — Token-optimised (preferred):**

Tighter prompt that carries more information (severity scale, strict output format, decision tree, false-positive short-circuit) in fewer tokens. The strict `Severity: <level>` output format ensures `parseSeverityFromResponse` extracts severity reliably instead of falling back to heuristic keyword scanning.
...
```
Step 1:  Migration — organizations, applications, app_agent_config       ✅ Done (see completions/phase-8-step-1-completion.md)
Step 2:  Migration — monitoring_state                                    ✅ Done (see completions/phase-8-steps-2-3-completion.md)
Step 3:  Queries — active apps, logs since cursor                        ✅ Done (see completions/phase-8-steps-2-3-completion.md)
         ↓
Step 4:  Lumber integration — classifier.go + severity gate              ✅ Done (see completions/phase-8-step-4-completion.md)
Step 5:  Monitor loop — monitor.go rewrite                               ✅ Done (see completions/phase-8-step-5-completion.md)
Step 6:  Monitoring system prompt                                        ✅ Done (bundled into Step 5 — Task 2)
Step 7:  RunMonitoring agent method                                      ✅ Done (bundled into Step 5 — Task 4)
         ↓
Step 8:  Wire startup/shutdown                                           ✅ Done (bundled into Step 5 — Tasks 6-7)
         ↓
Step 9:  (Convention only — no code)                                     ✅ Done (entry types emitted by monitor loop)
Step 10: Frontend — app model, config, dashboard, log badges             ✅ Done (see completions/phase-8-step-10-completion.md)
         ↓
Step 11: Tests                                                           ✅ Done (see completions/phase-8-step-11-completion.md)
```
