...
The agent is the intelligence at the centre of Heimdall. It is powered by an LLM with access to a defined set of tools (query database, search logs, inspect codebase, recall past incidents). It operates in three modes:
...
- **Scheduled mode** — the agent runs stored investigation prompts on a cron schedule. Useful for proactive probes against pull-only connectors (Postgres, GitHub) that the monitoring loop can't watch passively — e.g. "every hour, check pg_stat_statements for slow queries and summarise the worst offenders".

### Activity Feed

The activity feed is a curated, chronological master feed. It combines:
...
Within an organisation, each **application** is a first-class monitoring target — an independent unit of "what Heimdall is watching." Most users will run several: one per service, one per environment, or one per customer deployment. Every per-app surface (connections, agent configuration, activity feed, reports) is scoped to a single application, and switching applications is a single click in the sidebar selector. A new application can be created at any time via the wizard in that selector, and existing applications are managed from the Settings page.

Heimdall enforces that every organisation has at least one application, so the selector and the per-app pages always have something valid to render.
...
4. **Scheduled Investigations** — Stored prompts the agent runs on a cron schedule. Each run writes to the Activity feed; results can escalate to Reports. Good for periodic health checks, slow-query audits, and daily summaries.
5. **Activity** — The master chronological feed of all system activity and agent observations.
6. **Reports** — Incident reports generated by the agent, searchable and browsable.
7. **Settings** — Profile, organisation, and application management. Create new applications, delete existing ones (with explicit typed-to-confirm), and review account-level state. Settings is org-scoped rather than per-app; it's where you go to manage *which* applications exist, not what any individual application is doing.
